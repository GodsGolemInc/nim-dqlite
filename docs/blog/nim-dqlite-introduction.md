...
Worker 関数では `ptr`（ARC 追跡なし）、Poller では `ref`（ARC の `=destroy` に refcount デクリメントを任せる）に分離:

```nim
# Worker: ptr — ARC 追跡なし、フィールドアクセスのみ
let db = cast[ptr MasterDbObj](msg.dbPtr)

# Poller: ref — =destroy がスコープ終了時に refcount 2→1
let db = cast[MasterDb](res.msg.dbPtr)
```

`cast` は Nim の "shut up I know what I'm doing" 機能であり（nim-lang/Nim#25733 での Araq のフィードバック）、ARC のセマンティクスを理解した上で使うものだ。この発見は [ADR-0003](../adr/0003-arc-cursor-for-cast-pointer.md) に記録した。

**教訓**: `cast[ref T](pointer)` と `GC_ref` を組み合わせる場合、`=destroy` が発生する箇所を**一箇所に限定**する設計が鍵。Worker は `ptr` で ARC 追跡を回避し、Poller の `=destroy` だけが refcount をデクリメントすることで、手動 `GC_unref` が不要になる。
